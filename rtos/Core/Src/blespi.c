...
void txRxBleSpi(uint8_t *pTxData, uint8_t *pRxData) {
	HAL_SPI_Init(&hspi1);
	if (HAL_SPI_TransmitReceive(&hspi1, pTxData, pRxData, BLE_PKT_SIZ, 100)
			!= HAL_OK) {
		Error_Handler();
	}
	toggleBleStatusLed();
...
void sendBlePkt(uint8_t *req, uint8_t *eRxResp) {
	uint8_t pRxData[BLE_PKT_SIZ] = { 0 };
	txRxBleSpi(req, pRxData);
	if (eRxResp != NULL) {
		if (memcmp(eRxResp, pRxData, BLE_PKT_SIZ) != 0) {
			dbgPrintBuff(req, BLE_PKT_SIZ);
			dbgPrintBuff(pRxData, BLE_PKT_SIZ);
		}
	}
}

void sendBleATZCmd(void) {
	uint8_t pTxData[BLE_PKT_SIZ] = { 0x10, 0x00, 0x0a, 0x4, 0x41, 0x54, 0x5a,
			0x0a, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
			0xff, 0xff };
	uint8_t eRxResp[BLE_PKT_SIZ] = { 0x20, 0x00, 0x0a, 0x4, 0x4f, 0x4b, 0x5a,
			0x0d, 0x0a, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
			0xff, 0xff };
...
void blePrintStr(char *printStr, uint8_t siz) {
	uint8_t pTxData[BLE_PKT_SIZ] = { 0x10, 0x01, 0x0a, 0xff, 0xff, 0xff, 0xff,
	...
	uint8_t eRxResp[20] = { 0x20, 0x01, 0x0a, 0x00, 0xff, 0xff, 0xff, 0xff,
			0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
			0xff };
	sendBlePkt(pTxData, eRxResp);
}
...
void blePrintf(const char *fmt, ...) {
	...
	uint8_t siz = strnlen(printStr, sizeof(printStr));
	...
	for (int i = 0; i < nbPkt; ++i) {
		...
		uint8_t curStrSiz = strnlen(curStr, sizeof(curStr));
